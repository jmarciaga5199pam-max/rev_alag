            ...
            switch ($action) {
                ...
                case 'get_patient_details':
                    ...
                    if ($patient) {
                        ...
                        // Files uploaded by the parent for this patient
                        $uploaded_files = [];
                        if ($fstmt = mysqli_prepare(
                            $conn,
                            "SELECT id, original_filename, mime_type, file_size, description, created_at
                               FROM patient_files
                              WHERE patient_id = ?
                              ORDER BY created_at DESC"
                        )) {
                            mysqli_stmt_bind_param($fstmt, "i", $patient_id);
                            mysqli_stmt_execute($fstmt);
                            $fres = mysqli_stmt_get_result($fstmt);
                            while ($fres && ($frow = mysqli_fetch_assoc($fres))) {
                                $uploaded_files[] = $frow;
                            }
